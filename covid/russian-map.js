...
var RussianMap = function(options, regions) {
    ...
    var region_values = options.region_values;
    ...
    var getColor = function(region_value) {

        var value_levels = [0, 1, 3, 10, 50, 100];
        var colors = ["#fed976", "#feb24c", "#fd8d3c", "#fc4e2a", "#e31a1c", "#b10026"];

        for(let i = 0; i < value_levels.length; ++i ){
            if(region_value <= value_levels[i])
                return colors[i];
        }
    ...
    for (var i in regions) {
        regions[i].attr = {
            region_value: region_values[regions[i].ident],
            fill: getColor(region_values[regions[i].ident])};
